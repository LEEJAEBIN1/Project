...
Controller::Controller(Ssd &parent):
	ssd(parent)
{
	ftl = new FtlImpl_Fast(this, ssd, ssd.ram);
	return;
}
...
enum status Controller::event_arrive(Event &event)
{
	if(event.get_event_type() == READ)
		return ssd.ram.read(event, *ftl);
	else if(event.get_event_type() == WRITE)
		return ssd.ram.write(event, *ftl);
	else
...
enum status Controller::issue(Event &event_list)
{
	...
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		...
		else if(cur -> get_event_type() == WRITE)
		{
			...
			ssd.get_tail(cur->get_address())->get_spare()->set_spare_lpn(cur->get_logical_address());
			ssd.get_tail(cur->get_address())->get_spare()->set_spare_ppn(cur->get_address().get_linear_address());
			//printf("WRITE page's lpn = (spare[0] %d), ppn = (spare[1] %d)\n\n", ssd.get_tail(cur->get_address())->get_spare()->get_spare_lpn(), ssd.get_tail(cur->get_address())->get_spare()->get_spare_ppn());
		}
