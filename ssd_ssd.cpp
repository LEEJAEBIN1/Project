...
Ssd::Ssd(uint ssd_size): 
	size(ssd_size), 
	controller(*this), 
	ram(RAM_READ_DELAY, RAM_WRITE_DELAY, *this), 
	...
	last_erase_time(0.0)
{
	...
	if (PAGE_ENABLE_DATA)
	{
		...
		ulong eccPageSize = ((ulong)(BLOCK_SIZE * 16)) * (ulong)PAGE_SIZE;
#ifdef __APPLE__
		...
		ecc_page_data = mmap64(NULL,eccPageSize, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_NORESERVE, -1 ,0);
#endif
	...
	dltodp = (int*)malloc(sizeof(int) * SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE);
	for (int i = 0; i < SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE; ++i)
	{
		dltodp[i] = -1;
	}
	dltoep = (int*)malloc(sizeof(int) * SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE);
	for (int i = 0; i < SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE; ++i)
	{
		dltoep[i] = -1;
	}
...
Page* Ssd::get_tail(Address address)
{
	return data[address.package].get_tail(address);
}
